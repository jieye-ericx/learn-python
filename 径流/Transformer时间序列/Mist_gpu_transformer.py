...
def get_padding_mask(input):
    ...
    for pub in range(1970, 2020):
        if pub == 1970:
            ...
            input1.loc[input1['出版年'] == 1970, '1970':str(
                pub - 1)] = 1  # 1 转换为 bool 是 TRUE，表示掩码
...
def get_data():
    ...
    train_data, test_data = train_test_split(
        data, train_size=0.8, random_state=10)
    data.reset_index(inplace=True, drop=True)
    ...
    train_label = torch.from_numpy(
        np.array(train_data.iloc[:, output_window:]))
    ...
    train_padding = torch.from_numpy(
        np.array(train_padding.iloc[:, :-output_window]))
    test_padding = torch.from_numpy(
        np.array(test_padding.iloc[:, :-output_window]))

    train_sequence = torch.stack(
        (train_seq, train_label, train_padding), dim=1).type(torch.FloatTensor)
    test_data = torch.stack(
        (test_seq, test_label, test_padding), dim=1).type(torch.FloatTensor)
...
def get_batch(source, i, batch_size):
    ...
    padding = torch.stack(torch.stack([item[2]
                          for item in data]).chunk(input_window, 1))
...
class TransAm(nn.Module):
    def __init__(self, feature_size=512, num_layers=1, dropout=0):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(
            d_model=feature_size, nhead=8, dropout=dropout)
    ...
    def forward(self, src, src_padding):
        ...
        output = self.transformer_encoder(
            src, self.src_mask, self.src_key_padding_mask)  # , self.src_mask)
...
def plot_and_loss(eval_model, data_source, epoch):
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 1, eval_batch_size):
            data, target, key_padding_mask = get_batch(
            ...
            if calculate_loss_over_all_values:
                ...
                total_loss += criterion(output[-output_window:],
                                        target[-output_window:]).item()

            test_result = torch.cat((test_result, output[-1].squeeze(1).view(-1).cpu() * max1),
                                    0)  # todo: check this. -> looks good to me
            truth = torch.cat(
                (truth, target[-1].squeeze(1).view(-1).cpu() * max1), 0)
            ...
            truth1 = torch.cat(
                (truth1, target[-5:].squeeze(2).transpose(0, 1).cpu() * max1), 0)
...
def evaluate(eval_model, data_source):
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 1, eval_batch_size):
            data, targets, key_padding_mask = get_batch(
                data_source, i, eval_batch_size)
            output = eval_model(data, key_padding_mask)
            if calculate_loss_over_all_values:
                ...
                total_loss += len(data[0]) * criterion(output[-output_window:],
                                                       targets[-output_window:]).cpu().item()
...
for epoch in range(1, epochs + 1):
    ...
    if (epoch % 5 == 0):
        val_loss, tran_output, tran_true, tran_output5, tran_true5 = plot_and_loss(
            model, val_data, epoch)
